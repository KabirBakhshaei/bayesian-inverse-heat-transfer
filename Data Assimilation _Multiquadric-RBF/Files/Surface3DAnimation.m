...
cd('E:\SISSA\000 Ensemble\11 63   Multiquadric\Files')
...
aviObj = VideoWriter('3D Combined surface plot.avi', 'Motion JPEG AVI');
...
gifFilename = '3D Combined surface plot.gif';
...
for i = 1:length(timeInstants)
    ...
    %title(['True Heat Flux at Time Instant ', num2str(i)]);
    ...
    title(['Reconstructed Heat Flux at Time Instant ', num2str(timeInstants(i), '%.1f'), ' seconds using Multiquadric RBF']);
    axis([xmin xmax ymin ymax gtrue_min gtrue_max]);
    view(3);
...
disp('Animations saved as "3D Combined surface plot.avi" and "3D Combined surface plot.gif"');
